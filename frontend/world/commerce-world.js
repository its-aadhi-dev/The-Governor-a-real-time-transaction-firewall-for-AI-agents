...
export class CommerceWorld {
	constructor(
		...
		if (!container) {
			throw new Error(
				"Commerce world container is required.",
			);
		}

		if (!region?.id) {
			throw new Error(
				"Commerce world requires a region.",
			);
		}
	...
	createPaths() {
		...
		for (const [x, z] of [
			...
			const path = new THREE.Mesh(
				new THREE.BoxGeometry(0.45, 0.04, 6),
				material,
			);
	...
	createWorldMarkers() {
		...
		positions.forEach(([x, y, z], index) => {
			const merchant = this.merchants[index];
	...
	handlePointer(event) {
		...
		const objects = this.world.children.filter(
			(child) => child.userData?.type === "merchant-slot",
		);
		...
		const merchant = marker.userData.merchant;
		...
		this.selectedItem = merchant.items?.[0] || null;

		this.onMerchantSelected({
			merchantId: merchant.merchant_id,
			merchantName: merchant.display_name,
			itemId: this.selectedItem?.item_id || null,
			itemName: this.selectedItem?.item_name || null,
			basePrice: this.selectedItem?.base_price || null,
			regionId: this.region.id,
	...
	animate() {
		...
		this.world.rotation.y =
			Math.sin(this.clock.getElapsedTime() * 0.08) * 0.025;
